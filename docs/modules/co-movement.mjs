"use strict";
  ...
  function createPanelController(scope = globalThis, options = {}) {
    ...
    const deferredDelayMs = Math.max(0, Number(options.deferredDelayMs) || 120);
    let frame = 0;
    let deferredTimer = 0;
    ...
    function requestDeferred(delayMs = deferredDelayMs) {
      if (deferredTimer) clearTimer?.(deferredTimer);
      deferredTimer = setTimer?.(() => {
        deferredTimer = 0;
        request();
      }, Math.max(0, Number(delayMs) || deferredDelayMs)) || 0;
      return true;
    }
    ...
    return Object.freeze({
      ...
      request,
      requestDeferred,
      stats: () => Object.freeze({ ...counters, pending: Boolean(frame || deferredTimer) }),
    });
  }

  const coMovement = Object.freeze({
    buildSummary,
