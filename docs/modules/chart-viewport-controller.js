(function initThinkStockChartViewportController(globalScope) {
  "use strict";
  ...
  function createDataRangeCache(options = {}) {
    ...
    function get(element) {
      ...
      traces.forEach((trace) => {
        const first = findBoundary(trace.x, false);
        const last = findBoundary(trace.x, true);
  ...
  function createRangeSyncController(scope = globalScope, options = {}) {
  ...
  function buildRenderViewportPlan(options = {}) {
    ...
    let restoreAiForecastViewport = options.restoreAiForecastViewport || null;
    let revealAiForecastRange = options.revealAiForecastRange === true;
    let trimAiForecastRange = options.trimAiForecastRange === true;
    ...
    if (autoChartReset && pendingCompositionViewport && nextVisibleDataRange) {
    ...
    } else if (!autoChartReset) {
      pendingCompositionViewport = null;
    }

    if (restoreAiForecastViewport && !showAiForecast && nextVisibleDataRange) {
      const restoredRange = clampRangeToData(
        restoreAiForecastViewport.range,
        nextVisibleDataRange,
      );
      if (restoredRange) {
        savedXRange = toIsoRange(restoredRange);
        pinnedXRange = [...savedXRange];
        userViewportPinned = restoreAiForecastViewport.userViewportPinned === true;
      }
      restoreAiForecastViewport = null;
      trimAiForecastRange = false;
    }
    ...
    if (trimAiForecastRange && !showAiForecast) {
      ...
      if (Number.isFinite(observedEndMs) && Number.isFinite(savedEndMs) && savedEndMs > observedEndMs) {
        ...
        const nextStartMs = Number.isFinite(savedStartMs) && savedStartMs < observedEndMs
          ? savedStartMs
          : toMilliseconds(observedStart);
        savedXRange = toIsoRange([nextStartMs, observedEndMs]);
        pinnedXRange = [...savedXRange];
      }
      trimAiForecastRange = false;
    }
  ...
  globalScope.ThinkStockChartViewportController = Object.freeze({
...
(function initThinkStockChartLayoutPolicy(globalScope) {
  "use strict";
  ...
  globalScope.ThinkStockChartLayoutPolicy = Object.freeze({ applyContainer, resolve });
}(typeof self !== "undefined" ? self : globalThis));

// Composition capture is part of viewport policy.
(function initThinkStockChartCompositionViewport(globalScope) {
  "use strict";
  ...
  globalScope.ThinkStockChartCompositionViewport = Object.freeze({
