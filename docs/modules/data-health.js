(function initThinkStockDataHealth(globalScope) {
  ...
  const seriesTimelinePolicy = globalScope.ThinkStockSeriesTimelinePolicy;
  const toNum = (value) => (
  ...
  function detectSeriesGaps(rows, policies = {}, options = {}) {
    ...
    Object.entries(policies || {}).forEach(([key, rawPolicy]) => {
      ...
      if (referenceDates.length && dates.length
        && typeof seriesIntegrity?.planSeriesRepairDates === "function") {
        const exactMissingDates = seriesIntegrity.planSeriesRepairDates(
          rows,
          key,
          referenceDates.at(-1),
          {
  ...
  function buildFreshnessItems(configs, todayText) {
    return (Array.isArray(configs) ? configs : []).map((config) => {
      ...
      const policyStaleDays = seriesTimelinePolicy?.maximumAsOfAgeDays?.(config.keys);
