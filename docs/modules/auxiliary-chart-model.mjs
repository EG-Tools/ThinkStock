import contract from "./auxiliary-chart-contract.mjs";
import { finiteRangeBetween } from "./chart-adjustments.mjs";
  ...
  function finiteRangeExtrema(dates, valueSeries, startDate = "", endDate = "") {
    ...
    let count = 0;
    if (dateInfo.sorted && typeof finiteRangeBetween === "function") {
      safeSeries.forEach((values) => {
        const range = finiteRangeBetween(values, startIndex, endIndex);
        if (!range) return;
  ...
  function buildAuxiliaryViewportRanges(model = {}, range = [], options = {}) {
    ...
    const activePanels = options.activePanels && typeof options.activePanels === "object"
      ? options.activePanels
      : null;
    const panelIsActive = (key) => !activePanels || activePanels[key] !== false;
    const emptyRange = () => ({
      minimum: Number.POSITIVE_INFINITY,
      maximum: Number.NEGATIVE_INFINITY,
      count: 0,
    });
    const adrRange = panelIsActive("adr") ? mergeExtrema(
      finiteRangeExtrema(
    ...
    const newsRange = panelIsActive("newsSentiment") ? finiteRangeExtrema(
      model.newsDates,
      [model.newsValues],
      startDate,
      endDate,
    ) : emptyRange();
    const vkospiRange = panelIsActive("vkospi") ? finiteRangeExtrema(
      model.vkospiDates,
      [model.vkospiValues],
      startDate,
      endDate,
    ) : emptyRange();
    const vixRange = panelIsActive("vkospi") ? finiteRangeExtrema(
      model.vixDates,
      [model.vixValues],
      startDate,
      endDate,
  ...
  const auxiliaryChartModel = Object.freeze({
    AUXILIARY_PANEL_KEYS,
    AUXILIARY_CHART_CONFIG,
...
export default auxiliaryChartModel;
