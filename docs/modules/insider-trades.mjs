...
import { buildEventMarkerTextFont } from "./chart-render-contract.mjs";
  ...
  const BUY_MARKER_TEXT = "▲";
  const SELL_MARKER_TEXT = "▼";
  const MARKER_TEXT_SIZE = 15;
  ...
  function mergeRowsWithChange(existing, incoming) {
    const before = sanitizeRows(existing);
    const rows = new Map();
    [...before, ...sanitizeRows(incoming)].forEach((row) => {
    ...
    return {
      rows: merged,
      changed: JSON.stringify(before) !== JSON.stringify(merged),
      added: Math.max(0, merged.length - before.length),
    };
  }

  function mergeRows(existing, incoming) {
    return mergeRowsWithChange(existing, incoming).rows;
  }
  ...
  function buildInsiderPopoverGroup(group) {
    const isBuy = group?.side !== "sell";
    const label = isBuy ? "매수" : "매도";
    return {
      name: group?.name || group?.ticker || "내부거래",
      plotDate: group?.plotDate || "",
      events: (group?.events || []).map((event) => {
        const owned = event.sharesOwned === null || event.sharesOwned === undefined
          ? ""
          : ` · 거래 후 ${formatShares(event.sharesOwned)}`;
        const ownershipRate = event.ownershipRate === null || event.ownershipRate === undefined
          ? ""
          : ` (${Number(event.ownershipRate).toLocaleString("ko-KR")}%)`;
        const caption = [
          event.reporter,
          event.role,
          meaningfulTransactionMethod(event.transactionMethod),
        ].filter(Boolean).join(" · ");
        return {
          title: `내부자거래 : ${label} · 보유 증감 ${formatShares(event.sharesChanged)}${owned}${ownershipRate}`,
          caption,
          tone: isBuy ? "insider-buy" : "insider-sell",
          url: event.url || "",
        };
      }),
    };
  ...
  function markerTrace(groups, side, options = {}) {
    ...
    const hoverDetailTemplates = matches.map((group) => {
    ...
    return {
      ...
      text: matches.map(() => isBuy ? BUY_MARKER_TEXT : SELL_MARKER_TEXT),
      ...
      meta: {
        ...
        eventGroups: matches.map(buildInsiderPopoverGroup),
      },
    };
  }

  function buildMarkerTraces(groups, options = {}) {
    return [
      markerTrace(groups, "buy", options),
      markerTrace(groups, "sell", options),
    ].filter(Boolean);
  }
