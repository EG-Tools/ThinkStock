...
import { chartTraceOverlayKind } from "./chart-render-contract.mjs";
  ...
  function createAiForecastTraces(options = {}) {
    ...
    const inputCache = inputCacheModule?.createAiForecastInputCache({ maxEntries: 24 });
    const priceRevisionCache = typeof fingerprintDatedSeries === "function"
      ? inputCacheModule?.createSeriesRevisionCache({
    ...
    async function build(rows, seriesModels) {
      ...
      for (const item of workItems) {
        ...
        const requestedSeriesScale = Number(chartSession.seriesScales?.[series] ?? 1);
        const appliedSeriesScale = Number.isFinite(requestedSeriesScale) ? requestedSeriesScale : 1;
        const appliedSeriesOffset = Number(chartSession.seriesOffsets?.[series]) || 0;
        ...
        SCENARIO_KEYS.forEach((scenarioKey) => {
          ...
          const seriesTransformBaseValues = typeof invertSeriesTransform === "function"
            ? invertSeriesTransform(
  ...
  const aiForecastTraces = Object.freeze({
...
export default aiForecastTraces;
