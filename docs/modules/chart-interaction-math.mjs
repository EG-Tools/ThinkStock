"use strict";
  ...
  function yValueToLocalPixelFromAxis(yAxis, value) {
    ...
    if (!yAxis || !Number.isFinite(numericValue)) return null;
    const convert = typeof yAxis.d2p === "function"
      ? yAxis.d2p.bind(yAxis)
      : (typeof yAxis.l2p === "function" ? yAxis.l2p.bind(yAxis) : null);
    if (convert) {
      const pixel = Number(convert(numericValue));
      if (Number.isFinite(pixel)) return Number(yAxis._offset || 0) + pixel;
    }
    ...
    return yAxis._offset + yAxis._length * (1 - ((numericValue - minimum) / span));
  }
  ...
  function buildLineHitIndex(traces, seriesKeys) {
    return (Array.isArray(traces) ? traces : []).map((trace, traceIndex) => {
      ...
      const times = getTraceTimeMsArray(trace);
      let timesSorted = times.every(Number.isFinite);
      for (let index = 1; timesSorted && index < times.length; index += 1) {
        if (times[index] < times[index - 1]) timesSorted = false;
      }
      return {
        trace,
        traceIndex,
        seriesKey,
        renderRevision: String(trace?.meta?.renderFingerprint || ""),
        xValues: trace.x,
        yValues: trace.y,
        times,
        timesSorted,
  ...
  function lineHitIndexMatches(index, traces, seriesKeys) {
    ...
    return index.every((entry) => (
      ...
      && entry.renderRevision === String(entry.trace?.meta?.renderFingerprint || "")
  ...
  function findNearestLineTarget(index, targetMs, localY, yAxis, tolerance, includeEntry = null) {
    ...
    index.forEach((entry) => {
      ...
      const visuallyTied = Math.abs(distance - bestDistance) <= 0.5;
  ...
  function findNearestMarkerTarget(
    index,
    localX,
    localY,
    xAxis,
    ...
    if (!Array.isArray(index) || !Number.isFinite(localX) || !Number.isFinite(localY)) return null;
    let best = null;
    let bestDistance = Number.POSITIVE_INFINITY;
    index.forEach((entry) => {
      if (entry.trace?.visible === "legendonly" || (includeEntry && !includeEntry(entry))) return;
      let pointIndexes = null;
      if (entry.timesSorted && Number.isFinite(targetMs) && typeof xAxis?.d2p === "function") {
        let low = 0;
        let high = entry.times.length;
        while (low < high) {
          const middle = (low + high) >>> 1;
          if (entry.times[middle] < targetMs) low = middle + 1;
          else high = middle;
        }
        pointIndexes = [];
        for (let pointIndex = low - 1; pointIndex >= 0; pointIndex -= 1) {
          const x = Number(xAxis._offset || 0) + Number(xAxis.d2p(entry.xValues[pointIndex]));
          if (!Number.isFinite(x) || x < localX - tolerance) break;
          if (x <= localX + tolerance) pointIndexes.push(pointIndex);
        }
        for (let pointIndex = low; pointIndex < entry.xValues.length; pointIndex += 1) {
          const x = Number(xAxis._offset || 0) + Number(xAxis.d2p(entry.xValues[pointIndex]));
          if (!Number.isFinite(x) || x > localX + tolerance) break;
          if (x >= localX - tolerance) pointIndexes.push(pointIndex);
        }
      }
      const candidates = pointIndexes || entry.xValues.map((_, pointIndex) => pointIndex);
      candidates.forEach((pointIndex) => {
        if (includePoint && !includePoint(entry, pointIndex)) return;
        const x = Number(xAxis?._offset || 0) + Number(xAxis?.d2p?.(entry.xValues[pointIndex]));
        const y = yValueToLocalPixelFromAxis(yAxis, entry.numericY[pointIndex]);
        if (!Number.isFinite(x) || !Number.isFinite(y)) return;
        const distance = Math.hypot(x - localX, y - localY);
        if (distance <= tolerance && distance < bestDistance) {
          bestDistance = distance;
          best = { traceIndex: entry.traceIndex, seriesKey: entry.seriesKey, pointIndex };
        }
      });
    });
    return best;
