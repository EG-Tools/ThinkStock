(function initThinkStockSettingsPanelRuntime(globalScope) {
  "use strict";

  function createSettingsPanelRuntime(scope = globalScope, options = {}) {
    const {
      ADMIN_ACCESS_MASK,
      APP_BUILD_VERSION,
      APP_VERSION,
    ...
    if (!scope.document || !apiPeriodsModule || !releaseNotesModule
    ...
    function setup(msgEl) {
      ...
      const syncCursorLineModeUi = () => {
        ...
        cursorLineModeButtons.forEach((button) => {
          const active = button.dataset.chartCursorMode === activeMode;
