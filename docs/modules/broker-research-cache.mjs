...
import { cacheRefreshDecision } from "../../shared/runtime-freshness-policy.mjs";
  ...
  const freshnessPolicy = Object.freeze({ cacheRefreshDecision });
  ...
  function createDefaultPdfExtractor(scope, options, parser) {
    ...
    function loadTextLineModule() {
      if (!textLineModuleTask) {
        textLineModuleTask = import("./pdf-text-lines.mjs").catch((error) => {
          textLineModuleTask = null;
          throw error;
        });
      }
      return textLineModuleTask;
    }
    return async (bytes, metadata) => {
      ...
      const [pdfjs, textLineModule] = await Promise.all([
        loadPdfModule(),
        loadTextLineModule(),
      ]);
      ...
      try {
        ...
        for (let pageNumber = 1; pageNumber <= maximum; pageNumber += 1) {
          ...
          pages.push({
            page: pageNumber,
            lines: textLineModule.lineItemsFromTextContent(textContent),
          });
...
function createBrokerReportClient(scope = globalThis, options = {}) {
...
function createBrokerResearchCache(scope = globalThis, options = {}) {
  ...
  const brokerResearchCache = Object.freeze({
    CACHE_SCHEMA,
...
export { brokerResearchCache };
export default brokerResearchCache;
