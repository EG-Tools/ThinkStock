import {
  ...
  function createTaskProgress(scope = globalThis, options = {}) {
    const getRoot = options.getRoot || (() => null);
    const getText = options.getText || (() => null);
    const getBar = options.getBar || (() => null);
    ...
    function resolveAnchor(key, label = "") {
      return String(resolveTaskAnchor(key, label) || "");
    }
    ...
    function paint() {
      ...
      view.paint(value, current?.label ? `${current.label} ${value}%` : `${defaultLabel} ${value}%`);
      return value;
    }
    ...
    function begin(key, label = defaultLabel) {
    ...
    function complete(key, label = defaultLabel) {
      const taskKey = String(key || "").trim();
      const task = tasks.get(taskKey);
      if (!task) return false;
      task.progress = 1;
      task.done = true;
      task.label = String(label || task.label || defaultLabel);
      paint();
    ...
    return Object.freeze({ begin, cancel, complete, dispose, snapshot, update });
  }

  function createDisclosureProgress(scope = globalThis, options = {}) {
    return createTaskProgress(scope, {
      defaultLabel: "공시",
      getRoot: options.getRoot || (() => scope.document?.getElementById("disclosureProgress")),
      getText: options.getText || (() => scope.document?.getElementById("disclosureProgressText")),
      getBar: options.getBar || (() => scope.document?.getElementById("disclosureProgressBar")),
      ...options,
      resolveAnchor: options.resolveAnchor || ((key, label) => {
  ...
  function createApplicationProgressRuntime(scope = globalThis) {
    const byId = (id) => () => scope.document?.getElementById(id);
    return Object.freeze({
      disclosure: createDisclosureProgress(scope),
      eps: createTaskProgress(scope, {
        defaultLabel: "EPS",
        getRoot: byId("epsProgress"),
        getText: byId("epsProgressText"),
        getBar: byId("epsProgressBar"),
        anchor: "eps",
      }),
      signal: createTaskProgress(scope, {
        defaultLabel: "신호 계산중",
        getRoot: byId("signalProgress"),
        getText: byId("signalProgressText"),
        getBar: byId("signalProgressBar"),
        anchor: "signal",
        revealDelayMs: 0,
      }),
    });
