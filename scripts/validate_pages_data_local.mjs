...
async function validateDirectory(dataDir, label) {
  ...
  const macroRows = (rowsByDataset.get("macro_data") || [])
    ...
    .sort((left, right) => String(left.date).localeCompare(String(right.date)));
  assert.ok(macroRows.length >= 5000,
    `${label} bundled news sentiment history is incomplete (${macroRows.length})`);
  assert.ok(String(macroRows[0]?.date || "9999-12-31") <= "2005-01-10",
    `${label} bundled news sentiment starts too late (${macroRows[0]?.date || "missing"})`);
