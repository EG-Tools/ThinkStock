...
function bundleHeader() {
  ...
  const plotlyLock = lock.packages?.["node_modules/plotly.js"] || {};
  const fingerprint = crypto.createHash("sha256")
    .update(fs.readFileSync(__filename, "utf8").replace(/\r\n/g, "\n"))
    .update(JSON.stringify({ version: plotlyLock.version, integrity: plotlyLock.integrity }))
    .digest("hex")
    .slice(0, 16);
  return `/*! plotly.js (ThinkStock scatter-only) v2.35.2 | build ${fingerprint} | MIT */\n`;
}

function reusableBundle(header) {
  if (!fs.existsSync(target)) return false;
  const size = fs.statSync(target).size;
  if (size >= 950_000) return false;
  const prefix = fs.readFileSync(target, { encoding: "utf8", flag: "r" }).slice(0, header.length);
...
async function main() {
  ...
  if (reusableBundle(header)) {
    console.log(`Reused ${path.relative(root, target)} (${fs.statSync(target).size} bytes)`);
    return;
  }
