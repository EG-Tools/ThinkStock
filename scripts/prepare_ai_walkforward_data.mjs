...
const requestedPerMarket = perMarketArgument >= 0
  ...
  : 10;
const TARGET_PER_MARKET = Math.max(5, Math.min(50, Math.trunc(requestedPerMarket || 10)));
...
async function gatewayRows(endpoint, token) {
  if (!token) return [];
  try {
    const payload = await fetchJson(`${GATEWAY_URL}${endpoint}`, {
      cache: "no-store",
...
async function prepareContext() {
  ...
  if (!refresh && existing?.format === "thinkstock-ai-walkforward-context-v1") return existing;
  const env = parseEnv(await readFile(LOCAL_ENV_PATH, "utf8").catch(() => ""));
  const [macroFile, creditFile, adrFile, gatewayMacro, gatewayCredit, crisisRows] = await Promise.all([
...
const [prices, context] = await Promise.all([preparePrices(), prepareContext()]);
