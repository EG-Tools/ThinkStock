...
class AiMarketModelTests(unittest.TestCase):
    ...
    def test_cross_sectional_split_is_deterministic_and_market_stratified(self) -> None:
        start = pd.Timestamp("2020-01-01")
        samples = [
            TrainingSample(
                ticker=f"{index:06d}.{'KS' if index < 10 else 'KQ'}",
                anchor_date=start,
                target_date=start + pd.Timedelta(days=20),
                features=tuple([0.0] * len(FEATURE_NAMES)),
                target=0.0,
                baseline=0.0,
                volatility=0.02,
        ...
        development, holdout, tickers = split_cross_sectional_samples(samples)
        second_tickers = split_cross_sectional_samples(list(reversed(samples)))[2]

        self.assertEqual(tickers, second_tickers)
        self.assertEqual(len(tickers), 4)
        self.assertEqual({ticker.rsplit(".", 1)[-1] for ticker in tickers}, {"KS", "KQ"})
        self.assertFalse({item.ticker for item in development} & {item.ticker for item in holdout})

    def test_cross_sectional_holdout_accepts_a_shared_unseen_ticker_signal(self) -> None:
        samples = []
        start = pd.Timestamp("2018-01-01")
        for date_index in range(120):
            anchor = start + pd.Timedelta(days=date_index * 7)
            for ticker_index in range(12):
                signal = ((date_index % 13) - 6) / 6 + ((ticker_index - 5) * 0.02)
                features = [0.0] * len(FEATURE_NAMES)
                features[0] = signal
                target = signal * 0.05
                samples.append(TrainingSample(
                    ticker=f"{ticker_index:06d}.{'KS' if ticker_index < 6 else 'KQ'}",
                    anchor_date=anchor,
                    target_date=anchor + pd.Timedelta(days=35),
                    features=tuple(features),
                    target=target,
        ...
        selected = train_horizon_model(development, 20)
        selected["selected_lookback_years"] = 5

        result = evaluate_cross_sectional_holdout(development, holdout, 20, selected)

        self.assertTrue(result["ticker_disjoint"])
        self.assertTrue(result["passed"])
        self.assertGreater(result["metrics"]["improvement"], 0.8)
        self.assertGreater(result["metrics"]["direction_accuracy"], 0.9)
