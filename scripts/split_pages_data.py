...
def sync_segmented_mirror(data_dir: Path, mirror_dir: Path) -> int:
    manifest_path = data_dir / MANIFEST_FILE
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    files = {
        str(descriptor.get("file") or "")
        for dataset in (manifest.get("datasets") or {}).values()
        if isinstance(dataset, dict)
        for descriptor in dataset.values()
        if isinstance(descriptor, dict) and descriptor.get("file")
    }
    mirror_dir.mkdir(parents=True, exist_ok=True)
    for filename in sorted(files):
        shutil.copyfile(data_dir / filename, mirror_dir / filename)
    shutil.copyfile(manifest_path, mirror_dir / MANIFEST_FILE)
    return len(files)
