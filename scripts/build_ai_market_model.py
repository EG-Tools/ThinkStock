...
MODEL_FORMAT = "thinkstock-ai-market-model-v3"
...
EPSILON = 1e-9
CROSS_SECTIONAL_HOLDOUT_FRACTION = 0.2
...
def split_cross_sectional_samples(
    samples: Sequence[TrainingSample],
    holdout_fraction: float = CROSS_SECTIONAL_HOLDOUT_FRACTION,
) -> tuple[list[TrainingSample], list[TrainingSample], list[str]]:
    by_market: dict[str, set[str]] = {}
    for sample in samples:
        market = sample.ticker.rsplit(".", 1)[-1]
        by_market.setdefault(market, set()).add(sample.ticker)
    holdout_tickers: set[str] = set()
    for market, tickers in sorted(by_market.items()):
        ranked = sorted(
            tickers,
            key=lambda ticker: hashlib.sha256(
                f"{RANDOM_FEATURE_SEED}|{market}|{ticker}".encode("utf-8")
            ).hexdigest(),
        )
        if len(ranked) < 2:
            continue
        holdout_count = min(
            len(ranked) - 1,
            max(1, int(round(len(ranked) * clamp(holdout_fraction, 0.05, 0.4)))),
        )
        holdout_tickers.update(ranked[:holdout_count])
    development = [sample for sample in samples if sample.ticker not in holdout_tickers]
    holdout = [sample for sample in samples if sample.ticker in holdout_tickers]
    if not development or not holdout:
        raise ValueError("cross-sectional holdout requires at least two training tickers")
    return development, holdout, sorted(holdout_tickers)
...
def evaluate_cross_sectional_holdout(
    development_samples: Sequence[TrainingSample],
    holdout_samples: Sequence[TrainingSample],
    horizon: int,
    selected_model: dict,
    *,
    fold_count: int = 3,
) -> dict:
    validation_dates = common_recent_validation_dates(holdout_samples)
    blocks = [list(block) for block in np.array_split(validation_dates, fold_count) if len(block)]
    actual_all: list[float] = []
    predicted_all: list[float] = []
    baseline_all: list[float] = []
    winning_folds = 0
    completed_folds = 0
    lookback_years = int(selected_model["selected_lookback_years"])
    feature_transform = selected_model.get("feature_transform")
    ridge_lambda = float(selected_model["lambda"])
    blend_weight = float(selected_model["blend_weight"])
    for block in blocks:
        validation_start = pd.Timestamp(block[0])
        validation_end = pd.Timestamp(block[-1])
        training_start = validation_start - pd.DateOffset(years=lookback_years)
        training = [
            sample for sample in development_samples
            if sample.target_date < validation_start and sample.anchor_date >= training_start
        ]
        validation = [
            sample for sample in holdout_samples
            if validation_start <= sample.anchor_date <= validation_end
        ]
        if len(training) < 200 or not validation:
            continue
        model = fit_ridge(
    ...
    if completed_folds < 2 or not actual_all:
        raise ValueError(f"insufficient {horizon}-day cross-sectional validation folds")
    metrics = prediction_metrics(
        np.asarray(actual_all),
        np.asarray(predicted_all),
        np.asarray(baseline_all),
    )
    required_winning_folds = max(2, completed_folds - 1)
    passed = (
        metrics["improvement"] > 0
        and metrics["direction_accuracy"] >= 0.5
        and winning_folds >= required_winning_folds
    )
    ...
    return {
        ...
        "development_tickers": len({sample.ticker for sample in development_samples}),
        "holdout_tickers": len({sample.ticker for sample in holdout_samples}),
        "validation_samples": len(actual_all),
        "validation_folds": completed_folds,
...
def refit_horizon_model(samples: Sequence[TrainingSample], selected_model: dict) -> dict:
    lookback_years = int(selected_model["selected_lookback_years"])
    final_samples = samples_within_lookback(samples, lookback_years)
    if len(final_samples) < 200:
        raise ValueError(
            f"insufficient {selected_model['horizon_days']}-day final training samples: {len(final_samples)}"
        )
    final_model = fit_ridge(
        apply_feature_transform(
            np.asarray([item.features for item in final_samples]),
            selected_model.get("feature_transform"),
        ),
        np.asarray([item.target for item in final_samples]),
        float(selected_model["lambda"]),
    )
    ...
    result.update({
        ...
        "standard_deviations": [
            round(float(value), 10) for value in final_model["standard_deviations"]
        ],
...
def build_model_payload(
    ...
    for horizon in HORIZONS:
        ...
        development, holdout, _ = split_cross_sectional_samples(samples)
        selected = train_best_lookback_model(development, horizon)
        cross_sectional = evaluate_cross_sectional_holdout(
            development,
            holdout,
            horizon,
            selected,
        )
        model = refit_horizon_model(samples, selected)
        model["cross_sectional_holdout"] = cross_sectional
        model["reliability"] = round(min(
            float(model.get("reliability") or 0.0),
            float(cross_sectional.get("reliability") or 0.0),
        ), 6)
    ...
    return {
        ...
        "validation": {
            "method": "ticker-disjoint holdout after common recent three-fold purged walk-forward selection",
            "sample_step_trading_days": SAMPLE_STEP,
            "purge_rule": "training target_date must be earlier than validation anchor_date",
            "cross_sectional_holdout_fraction": CROSS_SECTIONAL_HOLDOUT_FRACTION,
            "cross_sectional_split": "deterministic SHA-256 split stratified by market",
            ...
            "selection_rule": "lowest development-set purged validation MAE, then highest direction accuracy",
            "publication_gate": "ticker-disjoint holdout beats baseline MAE with direction >= 0.5 in at least two folds",
...
def model_payload_passes_validation(payload: object) -> bool:
    ...
    for horizon in HORIZONS:
        ...
        try:
            ...
            return False
        cross_sectional = model.get("cross_sectional_holdout")
        cross_metrics = cross_sectional.get("metrics") if isinstance(cross_sectional, dict) else None
        if not isinstance(cross_metrics, dict):
            return False
        try:
            cross_mae = float(cross_metrics["mae"])
            cross_baseline_mae = float(cross_metrics["baseline_mae"])
            cross_improvement = float(cross_metrics["improvement"])
            cross_direction = float(cross_metrics["direction_accuracy"])
            cross_folds = int(cross_sectional["validation_folds"])
            cross_winning_folds = int(cross_sectional["winning_folds"])
            cross_tickers = int(cross_sectional["holdout_tickers"])
        except (KeyError, TypeError, ValueError):
            return False
        ...
        if (
            ...
            or cross_sectional.get("ticker_disjoint") is not True
            or cross_sectional.get("passed") is not True
            ...
            or cross_mae > cross_baseline_mae
            or cross_improvement <= 0
            or cross_direction < 0.5
            or cross_folds < 2
            or cross_winning_folds < max(2, cross_folds - 1)
            or cross_tickers < 2
