...
test("release notes keep major versions newest-first", () => {
  assert.equal(RELEASES[0].version, "2.77");
  assert.ok(RELEASES[0].items.some((item) => item.includes("품질")));
...
test("right navigation moves to older releases and left returns newer", () => {
  ...
  assert.equal(navigator.current().release.version, "2.77");
  assert.equal(navigator.current().hasNewer, false);

  const older = navigator.older();
  assert.equal(older.release.version, "2.76");
  assert.equal(older.hasNewer, true);

  assert.equal(navigator.newer().release.version, "2.77");
});
